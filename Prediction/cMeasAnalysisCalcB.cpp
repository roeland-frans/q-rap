...
/*				if (tClutter!=mMeasPoints[i].sClutter)
				...
				if (fabs(mMeasPoints[i].sPathLoss) > 200)
				{
					cout << endl; 
...
bool cMeasAnalysisCalc::OptimiseModelCoefAllTotal(unsigned MeasSource)
{
	...
	double Mean, MeanSquareError, StDev, CorrC, C1, C2;
...
*/
		...
		if (NumUsed > 0.05*TotalNumUsed/mPathLoss.mClutter.mNumber)
		{
			for (i=0; i<NUMTERMS; i++)
			{
				cout << "TERM" << i << "	Max="<< mMaxTerm[i] 
	...
	while ((!stop)&&(NumStop>0))
	{
		...
		for (i=1; i<mPathLoss.mClutter.mNumber; i++)
		{
			if (Change[i])
			{
				...
				if (Change[i]) CHeightDiff[i] = (cost-costOld)/DeltaH[i]*maxClutNum/(NumClut[i]+maxClutNum/10);
				else CHeightDiff[i] = 0.0;
				if ((fabs(cost-costOld) < 5e-5)&&(mPathLoss.mClutter.mClutterTypes[i].sHeight<0.05)&&(cost <= costMin))
				{
					Change[i] = false;  
					mPathLoss.mClutter.mClutterTypes[i].sHeight = 0.0;
				}
				if (fabs(CHeightDiff[i])<1.e-6)
				{ 
					if (first)
					{ 	
						Change[i] = false;
